...
export function Sign(pics,info){
    ...
    return (dispatch)=>{
        ...
        NetUtil.postForm(Constant.SignPhotoUrl,pics.pics[0],function (result) {
            dispatch({type: ActionTypes.SIGN_PHOTO1, msg: '提交照片数据1'});
            //console.log("正在提交照片");
            /**
             *
             {
            ...
            if((result.ackCode==='200' || result.ackCode===200)&& PHOTO_SIZE>1){

                //console.log('测试签到上传图片');

                NetUtil.postForm(Constant.SignPhotoUrl,pics.pics[1],function (result) {
                    dispatch({type:ActionTypes.SIGN_PHOTO2,msg:'提交照片数据2'});
                    //console.log("正在提交照片");
                    /**
                     *
                     {
                    ...
                    if((result.ackCode==='200' || result.ackCode===200) && PHOTO_SIZE>2){

                        //console.log('测试签到上传图片');


                        NetUtil.postForm(Constant.SignPhotoUrl,pics.pics[2],function (result) {
                            dispatch({type:ActionTypes.SIGN_PHOTO3,msg:'提交照片数据3'});
                            //console.log("正在提交照片");
                            /**
                             *
                             {
                            ...
                            NetUtil.postForm(Constant.SignInfoUrl,info,function (result) {
                                dispatch({type:ActionTypes.SIGN_INFO,msg:'提交签到信息数据'});
                                ...
                                if(result.ackCode==='200' || result.ackCode===200){
